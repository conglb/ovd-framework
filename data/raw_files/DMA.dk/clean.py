...
def clean(input_filename, output_filename):
    ...
    df_cleaned = df.dropna(subset=['# Timestamp','MMSI','Latitude','Longitude','Navigational status'])         # Xóa các hàng có giá trị bị thiếu
    df_cleaned = df_cleaned.drop_duplicates(subset=['# Timestamp','MMSI'])  # Xóa các hàng trùng lặp
